 ...
 * 告警处理专用 MatchRule：
 *
 * 与 `(pages)/settings/components/matchRule.tsx` 故意分离：
 *   - 这里只暴露 source_name（按名称匹配），不再使用 source_id
 *   - 其它 3 个共享入口（告警分派 / 屏蔽 / 富化）继续用共享版
 *
 * 为什么：业务口径"存 name 不存 ID"，告警处理（ActionRule.evaluate）后端
 * 已经支持 key='source_name' 与 id 自然匹配；用户只需配置 name。
 */
                      ...
                      <div className={styles.keySelect}>
                        ...
                        >
                          {(ruleOptions || ruleList)
                            .filter((item) =>
                              // 告警处理（alert 级）：去掉 source_id（保留 source_name，按"用 name 不用 ID"）；
                              // 同时去掉 location / service（这俩字段在 Event 模型上有，但 Alert 模型上没有，
                              // 是 event-only 的 ghost key，下拉里出现会导致匹配永远失败）。
                              item.name !== 'source_id' &&
                              item.name !== 'location' &&
                              item.name !== 'service'
                            )
                            .map((item) => {
                              // 告警处理：source_name 是唯一的"告警源"选项，去掉"(按名称)"后缀
                              const verboseName =
                                item.name === 'source_name' ? '告警源' : item.verbose_name;
                              return (

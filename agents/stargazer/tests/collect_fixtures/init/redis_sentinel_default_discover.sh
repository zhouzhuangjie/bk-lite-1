...
# Redis SENTINEL 采集 wrapper（G2.1,2026-07-07）
# -----------------------------------------------------------------------------
# 真实采集逻辑:复用 redis_default_discover.sh（600 行,内置 SENTINEL MASTERS /
# SENTINEL REPLICAS / CLUSTER NODES 全套采集）。本文件是它的镜像副本,只为了
# 在 catalog 里有独立的 init_script 路径和语义标识。
# 同步策略:与 init/redis_default_discover.sh 保持同步（来自 plugins/inputs/
# redis/redis_default_discover.sh）。如果上游脚本演进,本文件需同步更新。
#
# G2.1 patch 2026-07-07（仅本副本生效,不影响主脚本）:
# - parse_ports 重写:原版 `done | sort -nu` 在 alpine busybox ash 命令替换下
#   丢失多行输出（只返第一行）。改为 here-string + for 循环,直接 echo。
# - collect_instance 加 trace?（已移除,见 git log）
# - 其他函数未改,继承自上游镜像
#
# 触发 sentinel 分支的关键:
# 1. REDIS_TARGET_PORTS 含 sentinel 端口（26379）→ parse_ports 解析两端口
# 2. REDIS_DISCOVER_FROM_PROCESS=no → 关闭进程探测,避免只匹配 redis-server
#    命令行把 sentinel 端口吞掉
# 3. catalog.container_cmd 给 sentinel 也设 requirepass + sentinel auth-pass
#    → 否则 redis-cli 用 REDISCLI_AUTH 连接 sentinel 时 AUTH 失败混入 stdout
#
...
parse_ports() {
  # G2.1 2026-07-07: 重写为 here-string + here-doc 形式,避免 alpine sh subshell + sort
  # 管道丢行问题(原版 `done | sort -nu` 在 busybox ash 命令替换下只返第一行)。
  spec="${1:-}"
  _old_ifs="$IFS"; IFS=','
  for part in $spec; do
    IFS="$_old_ifs"
    ...
    case "$part" in
      *-*)
        start=${part%-*}; end=${part#*-}
        if [ "$start" -gt "$end" ] 2>/dev/null; then _t=$start; start=$end; end=$_t; fi
        p=$start
        while [ $p -le $end ]; do
        ...
        ;;
      *)
        if [ "$part" -ge 1 ] 2>/dev/null && [ "$part" -le 65535 ] 2>/dev/null; then
          echo "$part"
        fi
        ;;
    esac

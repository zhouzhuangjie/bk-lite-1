"""
OceanBase 数据库（蚂蚁集团分布式数据库）兼容性补丁集合。

OceanBase 基于 MySQL 协议，主要限制：
...
Migration 补丁：
- 位于 migrate_patch/patches/oceanbase/ 目录
...
def patch():
    """
    应用 OceanBase 数据库的常规补丁。

    这些补丁在 CoreConfig.ready() 中调用，
    用于修复 ORM 层面的兼容性问题。
    """
    # OceanBase 基于 MySQL 协议，大部分情况下兼容性较好
    # 目前主要问题是 JSON 列索引不支持，已通过 Migration 补丁处理
    _patch_jsonfield_contains_lookup()
    logger.info("OceanBase ORM patches applied (JSON contains lookup)")


def _patch_jsonfield_contains_lookup():
    """
    修复 OceanBase 数据库 JSONField 的 __contains 查询问题。

    问题：
    OceanBase 使用 MySQL 协议，JSON 类型支持有限。
    ...
    class OceanBaseJSONContains(Lookup):
        """OceanBase 数据库的 JSON 包含查询 Lookup"""
    ...
    # 注册新的 lookup（覆盖默认的）
    JSONField.register_lookup(OceanBaseJSONContains)
    logger.debug("JSONField.contains lookup patched for OceanBase")

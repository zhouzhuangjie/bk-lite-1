...
@pytest.fixture
def actor_context():
    def build(*group_ids, is_superuser=False, locale="zh-Hans"):
        return {
...
def test_get_mlops_module_data_unknown_module():
    ...
    assert result["message"] == "未知模块：nope"


def test_get_mlops_module_data_unknown_module_en():
    result = nats_api.get_mlops_module_data("nope", "x", 1, 10, 1, locale="en")
    assert result["result"] is False
    assert result["message"] == "Unknown module: nope"


def test_get_mlops_module_data_unknown_child():
    result = nats_api.get_mlops_module_data("dataset", "bad_child", 1, 10, 1)
    assert result["result"] is False
    assert result["message"] == "未知子模块：dataset/bad_child"


def test_get_mlops_module_data_unknown_child_en():
    result = nats_api.get_mlops_module_data("dataset", "bad_child", 1, 10, 1, locale="en")
    assert result["result"] is False
    assert result["message"] == "Unknown submodule: dataset/bad_child"
...
def test_get_mlops_module_data_root_filters_by_group(actor_context):
    ...
    result = nats_api.get_mlops_module_data("dataset", "anomaly_detection_dataset", 1, 10, 1, actor_context=actor_context(1))
...
def test_get_mlops_module_data_page_size_clamped_to_max(monkeypatch, actor_context):
    ...
    for i in range(5):
        AnomalyDetectionDataset.objects.create(name=f"d{i}", description="", team=[1])
    result = nats_api.get_mlops_module_data("dataset", "anomaly_detection_dataset", 1, 999, 1, actor_context=actor_context(1))
...
def test_get_mlops_module_data_page_size_floor_one(actor_context):
    ...
    result = nats_api.get_mlops_module_data("dataset", "anomaly_detection_dataset", 1, 0, 1, actor_context=actor_context(1))
...
def test_get_mlops_module_data_inherited_child_uses_dataset_team(actor_context):
    ...
    result = nats_api.get_mlops_module_data("dataset", "anomaly_detection_train_data", 1, 10, 1, actor_context=actor_context(1))
    assert result["count"] == 1
    assert result["items"][0]["name"] == "t-in"


def test_get_mlops_module_data_empty_result(actor_context):
    result = nats_api.get_mlops_module_data("dataset", "anomaly_detection_dataset", 1, 10, 999, actor_context=actor_context(999))
    assert result["result"] is True
...
def test_get_mlops_module_data_rejects_missing_actor_context_en():
    result = nats_api.get_mlops_module_data("dataset", "anomaly_detection_dataset", 1, 10, 1, locale="en")

    assert result == {"result": False, "message": "Missing caller context"}


def test_get_mlops_module_data_rejects_group_outside_actor_scope(actor_context):
    result = nats_api.get_mlops_module_data("dataset", "anomaly_detection_dataset", 1, 10, 2, actor_context=actor_context(1))

    assert result == {"result": False, "message": "无权访问该组织"}


def test_get_mlops_module_data_rejects_group_outside_actor_scope_en(actor_context):
    result = nats_api.get_mlops_module_data(
        ...
        10,
        2,
        actor_context=actor_context(1, locale="en"),
    )

    assert result == {"result": False, "message": "No permission to access this organization"}


def test_get_mlops_module_data_allows_superuser_group(actor_context):
    AnomalyDetectionDataset.objects.create(name="d2", description="", team=[2])
    result = nats_api.get_mlops_module_data("dataset", "anomaly_detection_dataset", 1, 10, 2, actor_context=actor_context(is_superuser=True))
